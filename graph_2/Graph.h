...
class Graph {
    private:
    unsigned int V;//num_nodes
    unsigned int E;//num_edges
    ...
    public:
         ...
         */
        void dfs(string start, double const & threshold);
         ...
         * Example: If our graph has edges "A"<-(0.1)->"B", "A"<-(0.5)->"C", "B"<-(0.1)->"C", and "C"<-(0.1)->"D", if we start at "A" and end at "D", we would return 0.1.
         ...
         */
        //void graphSearch(string start, string end_label);
        string find(unordered_map<string,string>& par, string const & i);
        void Union(unordered_map<string,string>& par, string const & x, string const & y);
